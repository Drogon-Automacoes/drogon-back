...
from app.api.deps import get_session, get_current_user, get_current_user_optional
...
def create_usuario(
    ...
    if current_user.tipo == TipoPerfil.SUPER_ADMIN:
        ...
        if not current_user.condominio_id:
             raise HTTPException(status_code=400, detail="Síndico sem condomínio não pode criar moradores.")
        usuario_dict["condominio_id"] = current_user.condominio_id

    else:
        raise HTTPException(status_code=403, detail="Você não tem permissão para cadastrar usuários.")
...
def list_usuarios(
    ...
    if current_user.tipo == TipoPerfil.ADMIN:
        ...
        query = select(Usuario).where(
            Usuario.condominio_id == current_user.condominio_id
...
def toggle_usuario_ativo(
    ...
    if current_user.tipo not in [TipoPerfil.ADMIN, TipoPerfil.SUPER_ADMIN]:
        raise HTTPException(status_code=403, detail="Apenas síndicos podem gerenciar moradores.")
